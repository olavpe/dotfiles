...
** Flycheck and Syntax stuff
   
   There are no flycheck specific configs (yet), other than the ~syntax-highlighting~
   layer has been enabled. Global settings can be set, but project specific
   settings can be set using a ~.dir-locals.el~. This will load the variables in
   this file and apply them to any buffer running within the same folder or any
   subfolder.
...
((python-mode
  ...
  (flycheck-python-pylint-executable . "~/gitRepos/python-sdk/venv/bin/python")))
#+end_src
...
**** Using pdf-tools as the default program to show latex files
